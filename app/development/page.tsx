      ...
      <section className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
        <Reveal>
          <SectionHeading
            ...
            description="LUN의 내부 실험 세부사항 대신, 모든 프로젝트에 공통으로 적용하는 판단 기준을 공개합니다."
          />
        </Reveal>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2">
            {developmentSection.portfolio.map((project, index) => (
              <Reveal key={project.name} delay={index * 100}>
                <article className="flex h-full flex-col rounded-(--radius-card) border border-white/15 bg-white/5 p-7 sm:p-8">
